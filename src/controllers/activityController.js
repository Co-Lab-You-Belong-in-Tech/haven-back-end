...
class ActivityController {
  static async getAllActivities(req, res) {
    try {
      const activities = await pool.query("SELECT activities.id, title, content, budget, spots_open, spots_total, username, users.id, first_name, last_name, pronouns, location, avatar_url  FROM activities JOIN users ON activities.user_id = users.id");
